...
#import "LTNetworkHelper.h"
#import "AFNetworking.h"
#import "AFNetworkActivityIndicatorManager.h"

#ifdef DEBUG
#define LTLog(...) printf("[%s] %s [第%d行]: %s\n", __TIME__ ,__PRETTY_FUNCTION__ ,__LINE__, [[NSString stringWithFormat:__VA_ARGS__] UTF8String])
#else
#define LTLog(...)
#endif

@implementation LTNetworkHelper
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                networkStatus ? networkStatus(LTNetworkStatusUnknow) : nil;
                break;
            case AFNetworkReachabilityStatusNotReachable:
                networkStatus ? networkStatus(LTNetworkStatusNotReachable) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
                networkStatus ? networkStatus(LTNetworkStatusReachableViaWWAN) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
                networkStatus ? networkStatus(LTNetworkStatusReachableViaWiFi) : nil;
                break;
...
+ (NSURLSessionTask *)GET:(NSString *)url
    ...
    responseCache != nil ? responseCache([LTNetworkCache httpCacheWithUrl:url parameters:parameters]) : nil;
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:url parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        //对数据异步缓存
        responseCache !=nil ? [LTNetworkCache setHttpCache:responseObject URL:url parameters:parameters] : nil;
