...
#ifdef DEBUG
#define TMLog(...) printf("[%s] %s [第%d行]: %s\n", __TIME__ ,__PRETTY_FUNCTION__ ,__LINE__, [[NSString stringWithFormat:__VA_ARGS__] UTF8String])
#else
#define TMLog(...)
#endif

@implementation TMNetworkHelper
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
                networkStatus ? networkStatus(TMNetworkStatusUnknow) : nil;
                break;
            case AFNetworkReachabilityStatusNotReachable:
                networkStatus ? networkStatus(TMNetworkStatusNotReachable) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
                networkStatus ? networkStatus(TMNetworkStatusReachableViaWWAN) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
                networkStatus ? networkStatus(TMNetworkStatusReachableViaWiFi) : nil;
                break;
...
+ (NSURLSessionTask *)GET:(NSString *)url
    ...
    responseCache != nil ? responseCache([TMNetworkCache httpCacheWithUrl:url parameters:parameters]) : nil;
    ...
    NSURLSessionTask *sessionTask = [_sessionManager GET:url parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        //对数据异步缓存
        responseCache !=nil ? [TMNetworkCache setHttpCache:responseObject URL:url parameters:parameters] : nil;
